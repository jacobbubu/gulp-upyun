var DEFAULT_HOST, File, Glob, Minimatch, STAT_FALSE, STAT_TRUE, Stat, WAIT_FOR_DEL, WAIT_FOR_GET, WAIT_FOR_PUT, api, async, delayCall, getHttpParams, getHttpStream, glob2base, httpDel, httpGet, httpPut, md5, path, ref, request, sign, streamToBuffer, through, toArray, url;
...
delayCall = function(fn, delay, cb) {
  return fn(function(err, res, body) {
    if (err != null) {
      return cb(err);
    } else if (res.statusCode === 429) {
      return setTimeout(function() {
        return delayCall(fn, delay, cb);
      }, delay);
    } else {
...
httpGet = function(params, cb) {
  return delayCall(request.bind(this, params), WAIT_FOR_GET, cb);
};

httpPut = function(params, cb) {
  return delayCall(request.put.bind(request, params), WAIT_FOR_PUT, cb);
};

httpDel = function(params, cb) {
  return delayCall(request.del.bind(request, params), WAIT_FOR_DEL, cb);
};

getHttpStream = function(params, cb) {
  return request(params).on('response', function(res) {
    if (res.statusCode === 429) {
      return setTimeout(function() {
        return getHttpStream(params, cb);
      }, WAIT_FOR_GET);
    } else {
      return cb(null, this);
    }
  ...
  getOneFolderInfo = function(folder, cb) {
    ...
    getPagedResult = function(cb) {
      ...
      oneRequest = function(iter, initial, cb) {
        ...
        return httpGet(params, function(err, res, body) {
          var hasMore;
          if (err != null) {
            return cb(err);
          }
          ...
          hasMore = (iter != null) && (iter !== 'g2gCZAAEbmV4dGQAA2VvZg') && iter.toLowerCase() !== 'none';
...
api.putFile = function(startingFolder, file, opts, cb) {
  ...
  uploadToServer = function(length, data, cb) {
    ...
    return httpPut(params, function(err, res, body) {
      var result;
      if (err != null) {
        return cb(err);
      }
...
api.delFileOrDir = function(filePath, opts, cb) {
  ...
  return httpDel(params, function(err, res, body) {
    if (err != null) {
      return cb(err);
    }
